...
const EVM_CONFIG = {
    ...
    userPk : process.env.userPk,
    resolverPk:  process.env.resolverPk
}
...
export class swapEVMtoKADENA {
    ...
    constructor() {
        ...
        this.kadenaProvider = new JsonRpcProvider(KADENA_CONFIG.url, KADENA_CONFIG.chainId)
        ...
        this.kadenaResolver = new Wallet(KADENA_CONFIG.resolverPk as string, this.kadenaProvider)
    }
    ...
    async swapCrossChain(
        ...
        console.log('⚡️ Swapping Tokens from EVM -> Kadena [cross-chain] ')
        ...
        console.log(`\n[KADENA]`, `Created dst deposit for order ${orderHash} in tx ${tx.hash}`)
        ...
        console.log(`\n[KADENA] : `, `Withdraw & Claim Funds for User in Kadena.EscrowDst`);
    ...
    private async checkBalAllowanceAndApproveEVM(tokenAddress: string, evmUserAddress: string, amount: bigint) {
        console.log('\n[KADENA] : User approving token to Resolver')
        const userWallet = new Wallet(KADENA_CONFIG.userPk as string, this.kadenaProvider);
        ...
        let allowance = await tokenContract.allowance(evmUserAddress, KADENA_CONFIG.escrowDst);

        if (allowance < amount) {
            // console.log('🔓 Approving token')
            const approveTx = await tokenContract.approve(KADENA_CONFIG.escrowDst, MaxUint256);
            const receipt = await approveTx.wait();
            // console.log('✅ token Approved')
        }
        allowance = await tokenContract.allowance(evmUserAddress, KADENA_CONFIG.escrowDst)
