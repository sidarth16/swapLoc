...
export class swapHEDERAtoKADENA {
    ...
    constructor() {
        ...
        this.evmProvider = new JsonRpcProvider(HEDERA_CONFIG.url, HEDERA_CONFIG.chainId)
        this.kadenaProvider = new JsonRpcProvider(KADENA_DST_CONFIG.url, KADENA_DST_CONFIG.chainId)

        this.userWallet = new Wallet(HEDERA_CONFIG.userPk as string, this.evmProvider)
        this.resolverWallet = new Wallet(HEDERA_CONFIG.resolverPk as string, this.evmProvider)
    ...
    async swapCrossChain(
        ...
        console.log('⚡️ Swapping Tokens from HEDERA -> Kadena_20 [cross-chain] ')
        console.log(`\t(swapAmounts: ${makingAmount} -> ${takingAmount})`)

        // 1.  Check + Approve src tokens
        console.log("\n[HEDERA]: Check + Approve SRC tokens to Resolver")
        ...
        // 2. Create Order
        console.log("\n[HEDERA]: Creating HTLC CrossChainOrder ")
        ...
        console.log(`    srcChain: HEDERA`);
        console.log(`    dstChain: KADENA20`);


        // 5. Sign order
        console.log("\n[HEDERA]: Sign Order and Propagate to all Relayers")
        ...
        console.log('\n[HEDERA]: Resolver Initiates EscrowSrc ')
        let escrowSrc = new ethers.Contract(
            HEDERA_CONFIG.escrowSrc,
            escrowSrcAbi.abi,
        ...
        console.log("✅ EscrowSrc deployed in tx:", txLink(txSRC.hash,"hedera"));
        console.log(`\n[HEDERA]: Order ${orderHash} filled for ${makingAmount} in tx ${txSRC.hash}`)
        ...
        // 9.KadenaResolver Withdraws Funds for User from EscrowDST
        console.log(`\n[HEDERA] : `, `Withdraw & Claim Funds for User in Hedera.EscrowSrc`);
        tx = await escrowDst.claim(
            orderHash, //swapId
            secretBytes32
        )
        await tx.wait();
        console.log(`✅ Claim complete on Hedera`);
        console.log(`Tx: ${txLink(tx.hash,"hedera")}`)
    ...
    private async checkAndApproveTokens(tokenAddress: string, amount: bigint) {
        ...
        let allowance = await tokenContract.allowance(this.userWallet.getAddress(), HEDERA_CONFIG.escrowSrc);

        if (allowance < amount) {
                    console.log('Approving token to resolver')
                    const approveTx = await tokenContract.approve(HEDERA_CONFIG.escrowSrc, MaxUint256);
                    const receipt = await approveTx.wait();
                    console.log('✅ token Approved')
                }
                allowance = await tokenContract.allowance(this.userWallet.getAddress(), HEDERA_CONFIG.escrowSrc)
                console.log("Resolver allowance:", allowance.toString())
    }
